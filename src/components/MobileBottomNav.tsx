...
const APP_PATHS = ['/dashboard', '/calendar', '/community', '/settings'];
...
export default function MobileBottomNav() {
  ...
  if (!APP_PATHS.some(p => pathname === p || pathname.startsWith(p + '/'))) return null;

  const handleAdd = () => {
    if (pathname === '/dashboard') {
      // Dashboard listens for this event and opens the modal
      document.dispatchEvent(new CustomEvent('applyd:add'));
    } else {
      // Navigate to dashboard with ?add=1 query, dashboard opens modal on mount
    ...
    >
      <div className="flex items-center h-16 px-2">
        ...
        <div className="flex-1 flex flex-col items-center justify-center tap-compact min-h-0">
          ...
          <span className="text-[10px] font-medium mt-1 leading-none" style={{ color: 'var(--text-tertiary)' }}>Add</span>
        </div>
        ...
        <Link
          ...
          className="flex-1 flex flex-col items-center justify-center gap-1 tap-compact min-h-0"
        >
          <ProfileIcon active={pathname === '/settings'} />
          <span
            className="text-[10px] font-medium leading-none"
            style={{ color: pathname === '/settings' ? 'var(--accent-blue)' : 'var(--text-tertiary)' }}
          >
            Profile
          </span>
        </Link>
