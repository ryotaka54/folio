...
import { Download, Copy, Check, RotateCcw, ChevronDown, ChevronUp, Mic, History as HistoryIcon } from 'lucide-react';
...
function InterviewContent() {
  ...
  // Leaderboard consent state — reset on each new question
  ...
  async function advance() {
    ...
    if (currentIdx + 1 >= questions.length) {
      ...
      // Reset leaderboard consent for new question
            ...
            {sessionsLoading ? (
              ...
              <div style={{ textAlign: 'center', padding: '60px 0' }}>
                ...
                <button
                  ...
                  style={{ color: 'var(--accent-blue)', fontSize: 13, background: 'none', border: 'none', cursor: 'pointer' }}
                >
                          ...
                          >
                            <div style={{ borderTop: '1px solid var(--border-gray)', padding: '16px 18px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                                ...
                                <div key={qi} style={{ background: 'var(--surface-gray)', borderRadius: 10, padding: '12px 14px', borderLeft: `3px solid ${SCORE_COLOR(e.feedback.score)}` }}>
                                  ...
                                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                                    {e.feedback.strengths.map((str, i) => (
                                      <span key={i} style={{ fontSize: 11, padding: '3px 9px', borderRadius: 9999, background: 'var(--success-bg)', color: 'var(--green-success)', border: '1px solid var(--success-border)' }}>{str}</span>
                                    ))}
                                  </div>
                                </div>
                              ))}
                              ...
                              <motion.button
                                ...
                                style={{
                                  marginTop: 4, height: 40, borderRadius: 10, background: 'var(--accent-blue)',
        ...
        {phase === 'pick' && view === 'session' && (
          <motion.div key="pick" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            ...
            <div style={{
              ...
              background: 'radial-gradient(ellipse 60% 50% at 50% 0%, color-mix(in oklch, var(--accent-blue) 10%, transparent) 0%, transparent 70%)',
              textAlign: 'center',
            }}>
              <motion.div
            ...
            <div style={{ maxWidth: 900, margin: '0 auto', padding: '0 24px 80px' }}>
              ...
              {loading ? (
                ...
                <div style={{ textAlign: 'center', padding: '60px 0' }}>
                  ...
                  <Link href="/dashboard" style={{ color: 'var(--accent-blue)', fontSize: 13, textDecoration: 'none', marginTop: 8, display: 'inline-block' }}>Go add some →</Link>
                </div>
              ) : (
        ...
        {phase === 'setup' && selectedApp && (
          <motion.div key="setup" initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -24 }} style={{ maxWidth: 600, margin: '0 auto', padding: '48px 24px 80px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', marginBottom: 28, background: roastMode ? 'var(--error-bg)' : 'var(--surface-gray)', borderRadius: 10, border: roastMode ? '1px solid var(--error-border)' : '1px solid var(--border-gray)', transition: 'all 0.2s' }}>
              <div>
                <span style={{ fontSize: 13, fontWeight: 600, color: roastMode ? 'var(--error-text)' : 'var(--brand-navy)' }}>🔥 Brutal feedback</span>
              ...
              <div>
                ...
                <AnimatePresence mode="wait">
                  ...
                  >
                    ...
                    {error && <p style={{ fontSize: 13, color: 'var(--error-text)', marginTop: 12 }}>{error}</p>}
                  </motion.div>
                </AnimatePresence>
              </div>
            </div>
          ...
          >
            ...
            {feedback.roast_mode && (
              ...
              >
                ...
                <span style={{ fontSize: 12, color: 'var(--error-text)', fontWeight: 600 }}>Brutal mode — no diplomatic softening</span>
              </motion.div>
            )}
              ...
              {!lbPosted ? (
                <div style={{ border: '1px solid var(--border-gray)', borderRadius: 12, padding: '14px 16px', background: 'var(--surface-gray)' }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                    <button onClick={postToLeaderboard} disabled={!lbPostScore} style={{ height: 32, padding: '0 16px', borderRadius: 8, border: 'none', background: lbPostScore ? 'var(--accent-blue)' : 'var(--border-gray)', color: '#fff', fontSize: 13, fontWeight: 600, cursor: lbPostScore ? 'pointer' : 'default', fontFamily: 'inherit' }}>
                      Post
                    </button>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', border: '1px solid var(--success-border)', borderRadius: 10, background: 'var(--success-bg)', gap: 10, flexWrap: 'wrap' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ color: 'var(--green-success)', fontSize: 13 }}>✓ Posted</span>
                    <a href={`/leaderboard/${selectedApp.company.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`} target="_blank" rel="noopener noreferrer" style={{ fontSize: 12, color: 'var(--accent-blue)', textDecoration: 'none' }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    ...
                    <button onClick={() => manageLeaderboardEntry('delete')} style={{ fontSize: 11, padding: '4px 10px', borderRadius: 6, border: '1px solid var(--error-border)', background: 'transparent', color: 'var(--error-text)', cursor: 'pointer', fontFamily: 'inherit' }}>Remove</button>
                  </div>
                </div>
              ) : null}
            </motion.div>
            ...
            {feedback.strengths.length > 0 && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} style={{ marginBottom: 18 }}>
                <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', color: 'var(--green-success)', margin: '0 0 10px', fontFamily: "'DM Mono', monospace" }}>STRENGTHS</p>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>
                  {feedback.strengths.map((s, i) => (
                    <motion.span
                      ...
                      style={{ fontSize: 12, padding: '5px 12px', borderRadius: 9999, background: 'var(--success-bg)', color: 'var(--green-success)', border: '1px solid var(--success-border)' }}
                    >{s}</motion.span>
                  ))}
                </div>
              </motion.div>
            ...
            {feedback.improvements.length > 0 && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }} style={{ marginBottom: 24 }}>
                <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', color: 'var(--amber-warning)', margin: '0 0 10px', fontFamily: "'DM Mono', monospace" }}>TO IMPROVE</p>
